...
public class KpiC extends GenMethods {
	...
	@Test(priority = 1, groups = "KPI-C", alwaysRun = true)
	private void GIN81() throws InterruptedException //To verify the functionality of fields on "KPI-C" tab
	{
		clickWhenReady(mapSettingsBtn, 5);
		clickWhenReady(kpiCTab, 5);
		...
		for(int i=2; i <= availableTechnologies.size(); i++){
			...
			sendKeysToElement(kpiCStartDateField, startDateVal);
			sendKeysToElement(kpiCEndDateField, endDateVal);
			
			String columnName = xr.getCellData("CalenderInputDate", "KPI-C Technology", i);
			if(columnName.equalsIgnoreCase(techName)){
				xr.setCellData("CalenderInputDate", "KPI-C Start Date", i, startDateVal);
				xr.setCellData("CalenderInputDate", "KPI-C End Date", i, endDateVal);
			}

			List<WebElement> availableKpis = dr.findElements(kpiCKpisField);
			for(int j=1; j <= availableKpis.size(); j++){
				WebElement kpiValue = dr.findElement(By.xpath(xpathKpisFieldKpiC + "["+ j +"]"));
				kpiValue.click();
				...
				if(kpiValue.getText().equalsIgnoreCase("Please select a technology.")){
					...
					String thresholdValue = new Select(dr.findElement(kpiCThresholdSelector)).getFirstSelectedOption().getText();
					...
					for(int k=3; k <= thresholdValueNo+2; k++){
						String actThresholdVal = getElementText(By.xpath("//*[@id='kpi-c_settings_0']//*[@id='ksc-d_dd_0']/tbody/tr["+ k +"]/td[4]/div"), 5);
		...
		//List<WebElement> thresholdOptions = dr.findElements(By.xpath("//*[@id='kpi-c_threshold_num_0']/option"));
		for(int x=1; x <= thresholdValueNo; x++){
			int y = x - 1;
			if(x < thresholdValueNo){
				String randomNo = genData.generateRandomNumeric(3);
				sendKeysToElement(By.id("cr-c_v"+ y), randomNo);
				getElementText(By.id("cr-c_v_d_"+ x), 5);
				Assert.assertEquals(randomNo, getElementText(By.id("cr-c_v_d_"+ x), 5));
			}
			clickWhenReady(By.xpath("//*[@id='kpi-c_settings_kpi_crs_0']//*[@id='cr_c"+ y +"']"), 5);
			...
			clickWhenReady(By.xpath("//*[@id='kpi-c_settings_0']//*[@id='ksc_dd_0']/tbody/tr[3]/td[4]/div"), 5);
		}
		clickWhenReady(kpiCSaveBtn, 5);
	}

	@Test(priority = 2, groups = "KPI-C")
	public void GIN82() throws InterruptedException //To verify the functionality of "KPI-C" tab for technology 'CDMA1X'
	{
		dr.navigate().refresh();
		clickWhenReady(mapSettingsBtn, 5);
		clickWhenReady(kpiCTab, 5);

		List<WebElement> availableTechnologies = dr.findElements(kpiCTechField);
		for(int i=1; i <= availableTechnologies.size(); i++){
			WebElement techValue = dr.findElement(By.xpath(xpathtechFieldKpiC + "["+ i +"]"));
		...
		String columnName = xr.getCellData("CalenderInputDate", "KPI-C Technology", 2);
		if(columnName.equalsIgnoreCase("CDMA 1X")){
			startDateVal = xr.getCellData("CalenderInputDate", "KPI-C Start Date", 2);
			endDateVal = xr.getCellData("CalenderInputDate", "KPI-C End Date", 2);
		}
		sendKeysToElement(kpiCStartDateField, startDateVal);
		sendKeysToElement(kpiCEndDateField, endDateVal);
		...
		clickWhenReady(By.xpath(xpathKpisFieldKpiC + "["+ ranNum +"]"), 5);
		clickWhenReady(kpiCSaveBtn, 5);
		...
		for(int i=1; i <= actAvailableLayers.size(); i++){
			...
			if(valueselected == true){
				...
				Assert.assertEquals(actSelectedLayerText, "KPI-C");
				break;
			}
		}
	}

	@Test(priority = 3, groups = "KPI-C")
	public void GIN83() throws InterruptedException //To verify the functionality of "KPI-C" tab for technology 'CDMA Voice'
	{
		dr.navigate().refresh();
		clickWhenReady(mapSettingsBtn, 5);
		clickWhenReady(kpiCTab, 5);
		...
		String columnName = xr.getCellData("CalenderInputDate", "KPI-C Technology", 3);
		if(columnName.equalsIgnoreCase("CDMA Voice")){
			startDateVal = xr.getCellData("CalenderInputDate", "KPI-C Start Date", 3);
			endDateVal = xr.getCellData("CalenderInputDate", "KPI-C End Date", 3);
		}
		sendKeysToElement(kpiCStartDateField, startDateVal);
		sendKeysToElement(kpiCEndDateField, endDateVal);
	...
	@Test(priority = 4, groups = "KPI-C")
	public void GIN84() throws InterruptedException //To verify the functionality of "KPI-C" tab for technology 'EVDO'
	{
		dr.navigate().refresh();
		clickWhenReady(mapSettingsBtn, 5);
		clickWhenReady(kpiCTab, 5);
		...
		String columnName = xr.getCellData("CalenderInputDate", "KPI-C Technology", 4);
		if(columnName.equalsIgnoreCase("EVDO")){
			startDateVal = xr.getCellData("CalenderInputDate", "KPI-C Start Date", 4);
			endDateVal = xr.getCellData("CalenderInputDate", "KPI-C End Date", 4);
		}
		sendKeysToElement(kpiCStartDateField, startDateVal);
		sendKeysToElement(kpiCEndDateField, endDateVal);
	...
	@Test(priority = 5, groups = "KPI-C")
	public void GIN85() throws InterruptedException //To verify the functionality of "KPI-C" tab for technology 'LTE'
	{
		dr.navigate().refresh();
		clickWhenReady(mapSettingsBtn, 5);
		clickWhenReady(kpiCTab, 5);
		...
		String columnName = xr.getCellData("CalenderInputDate", "KPI-C Technology", 5);
		if(columnName.equalsIgnoreCase("LTE")){
			startDateVal = xr.getCellData("CalenderInputDate", "KPI-C Start Date", 5);
			endDateVal = xr.getCellData("CalenderInputDate", "KPI-C End Date", 5);
		}
		sendKeysToElement(kpiCStartDateField, startDateVal);
		sendKeysToElement(kpiCEndDateField, endDateVal);
	...
	@Test(priority = 6, groups = "KPI-C")
	public void GIN86() throws InterruptedException //To verify the threshold drop-down functionality in "KPI-C" tab
	{
		dr.navigate().refresh();
		clickWhenReady(mapSettingsBtn, 5);
		clickWhenReady(kpiCTab, 5);
		...
		clickWhenReady(By.xpath(xpathtechFieldKpiC + "["+ ranNum +"]"), 5);

		List<WebElement> availableKpis = dr.findElements(kpiCKpisField);
		int availableKpiCount = availableKpis.size();
		int ranNum1 = genData.generateRandomNumericFromRange(availableKpiCount + 1, 1);
		clickWhenReady(By.xpath(xpathKpisFieldKpiC + "["+ ranNum1 +"]"), 5);

		List<WebElement> thresholdOptions = dr.findElements(By.xpath("//*[@id='kpi-c_threshold_num_0']/option"));
		for(int z=1; z <= thresholdOptions.size(); z++){
			WebElement thresholdValue = dr.findElement(By.xpath("//*[@id='kpi-c_threshold_num_0']/option["+ z +"]"));
